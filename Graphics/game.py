...
def mousePress(w,x,y,b):
    if not w.gameStarted and b=="left" and w.numplayers>0 and w.namedplayers==w.numplayers and inbox(10,150,x,y,90,40):
        check()
        time.sleep(1)
        if confirm():
            w.gameStarted = True
            for con in w.connections:
                con.c.send("*start")
    elif not w.gameStarted and b=="left" and inbox(10,100,x,y,90,40):
        check()
        time.sleep(1)
        confirm()
    elif b=="left" and not w.gameStarted:
        ypos=10
        for p in w.players:
